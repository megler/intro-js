...
export function hintQuerySelectorAll(selector) {
  ...
  return hintsWrapper
    ? hintsWrapper.querySelectorAll(selector)
    : [];
}
 ...
 * Hide all hints
 *
 * hintsOff allows user to turn off hints and re-enable functionality of rest
 * of the page
 *
 * @api private
 * @method hideHints
...
export function hideHints() {
  ...
  const hintsOff = document.querySelector(".introjs-hints");
  forEach(hints, async hint => {
    await hideHint.call(this, hint.getAttribute("data-step"));
  });
  hintsOff.style.display = "none";
}
...
export async function addHints() {
  ...
  if (hintsWrapper === null) {
    hintsWrapper = createElement("div", {className: "introjs-hints"});
  }
  ...
  forEach(this._introItems, (item, i) => {
    ...
    const hint = createElement("a", {className: "introjs-hint"});
    ...
    const hintDot = createElement("div", {className: "introjs-hint-dot"});
    const hintPulse = createElement("div", {className: "introjs-hint-pulse"});

    //create button so user can turn off hints/re-enable page functionality
    const hintToggleButton = createElement("button", {className: "introjs-hint-toggle"});
    let buttonWrapper = document.querySelector(".introjs-hints-overlay");
    // When hints are active, create overlay that blocks page functionality except
    // for links and hintToggleButton
    buttonWrapper = createElement("div", {className: "introjs-hints-overlay"});
    buttonWrapper.style.display = "block";
    hintToggleButton.innerHTML = "Turn off Hints";

    hintToggleButton.onclick = function () {
      hideHints();
    };
    ...
    // Add hintToggle to DOM
    hintsWrapper.appendChild(hintToggleButton);
    hintsWrapper.appendChild(buttonWrapper);
  });
  ...
  if (this._options.hintAutoRefreshInterval >= 0) {
    this._hintsAutoRefreshFunction = debounce(() => reAlignHints.call(this), this._options.hintAutoRefreshInterval);
 ...
 */
export function alignHintPosition(position, {
  style
}, element) {
...
export async function showHintDialog(stepId) {
  ...
  const tooltipLayer = createElement("div", {className: "introjs-tooltip"});
  ...
  if (this._options.hints) {
    ...
    forEach(hints, currentElement => {
      ...
      this._introItems.push({
        ...
        hintPosition: currentElement.getAttribute("data-hint-position") || this._options.hintPosition,
        hintAnimation,
